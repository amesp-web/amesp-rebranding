...
export default function NewEventPage() {
  ...
  const addSimple = (set: any) => set((arr: any[]) => [...arr, { logo_url: "", name: "" }])
  const setSimple = (set: any, i: number, k: string, v: any) => set((arr: any[]) => arr.map((it, idx) => idx === i ? { ...it, [k]: v } : it))
  ...
  const onSubmit = async () => {
    ...
    setLoading(true)
    try {
      const res = await fetch('/api/admin/events', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...form, schedule, stands, participants, sponsors, published: !!form.published }) })
    ...
    <div className="space-y-6">
      ...
      <Card className="border-0 shadow-xl">
        <CardHeader>
          ...
          <CardDescription>Monte os dias e atrações do evento</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
      ...
      {[
        { title: 'Stands', state: stands, set: setStands },
        { title: 'Participantes', state: participants, set: setParticipants },
        { title: 'Patrocinadores', state: sponsors, set: setSponsors },
      ].map((blk, idx) => (
        <Card key={idx} className="border-0 shadow-xl">
          ...
          <CardContent className="space-y-3">
            <Button variant="outline" onClick={() => addSimple(blk.set)}>Adicionar</Button>
            {blk.state.map((it: any, i: number) => (
              <div key={i} className="grid grid-cols-1 md:grid-cols-6 gap-3 items-end">
                  ...
                  <Input
                    ...
                    onChange={async (e) => {
                      ...
                      try {
                        ...
                        setSimple(blk.set, i, 'logo_url', url)
                      } catch (err: any) {
                ...
                <div className="md:col-span-4">
                  <Label>Nome</Label>
                  <Input value={it.name} onChange={(e) => setSimple(blk.set, i, 'name', e.target.value)} />
                </div>
              </div>
            ))}
          </CardContent>
      ...
      <div className="flex justify-end gap-3">
        ...
        <Button onClick={onSubmit} disabled={loading}>{loading ? 'Salvando...' : 'Salvar evento'}</Button>
      </div>
    </div>
  )
}
