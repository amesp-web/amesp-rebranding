...
export default function AdminTurismoPage() {
  ...
  useEffect(() => {
    ;(async () => {
      const res = await fetch('/api/admin/turismo', { cache: 'no-store' })
      const data = await res.json()
      setTitle(data?.content?.title || 'Turismo no Litoral')
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/admin/turismo', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
  ...
  const iconHelp = 'Ícones: fish, users, waves, award, leaf, mapPin, camera, heart, factory, star, calendar'
    ...
    return (
      <div className="rounded-2xl ring-1 ring-black/5 p-5 bg-gradient-to-br from-card to-card/60 shadow-md space-y-3">
        ...
        <div>
          ...
          >
            {["fish", "users", "waves", "award", "leaf", "mapPin", "camera", "heart", "factory", "star", "calendar"].map((k) => (
    ...
    <div className="space-y-8">
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-blue-600 via-cyan-500 to-teal-400 p-8 shadow-xl">
        <div className="relative">
          <h1 className="text-3xl font-bold text-white mb-2">Turismo</h1>
          <p className="text-blue-50 text-lg">Gerencie os textos da seção Turismo na Home</p>
        </div>
      </div>
      ...
      <Card className="border-0 shadow-xl bg-gradient-to-br from-blue-50/30 to-teal-50/30 ring-1 ring-black/5 rounded-2xl">
        <CardHeader>
          ...
          <CardDescription>Título e subtítulo exibidos na seção Turismo</CardDescription>
        </CardHeader>
      ...
      <Card className="border-0 shadow-xl">
        <CardHeader className="border-b bg-gradient-to-r from-blue-50 to-cyan-50">
          <div className="flex items-center justify-between">
            <div>
              <CardTitle>Conteúdo Completo (Modal Turismo)</CardTitle>
              <CardDescription className="mt-1">
                Conteúdo rico exibido no modal &quot;Saiba mais&quot; da seção Turismo
              </CardDescription>
            </div>
          </div>
        </CardHeader>
